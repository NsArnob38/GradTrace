    ...
    return (
        <div className="min-h-screen bg-bg">
            ...
            <div className="max-w-2xl mx-auto px-6 py-8">
                ...
                <p className="text-muted text-sm mb-8">Update your profile information</p>

                <div className="bg-white rounded-xl shadow-sm p-6 space-y-5">
                    {/* Full Name */}
                    <div>
                        <label className="text-xs text-muted uppercase tracking-wider font-medium block mb-1.5">Full Name</label>
                        <input
                            type="text"
                            value={form.full_name}
                            onChange={e => setForm({ ...form, full_name: e.target.value })}
                            placeholder="Enter your full name"
                            className="w-full border border-border rounded-lg px-4 py-2.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-accent/30 focus:border-accent transition-all"
                        />
                    </div>

                    {/* Student ID */}
                    <div>
                        <label className="text-xs text-muted uppercase tracking-wider font-medium block mb-1.5">Student ID</label>
                        <input
                            type="text"
                    ...
                    <div className="pt-2">
                        <button
                            onClick={handleSave}
                            disabled={saving}
                            className="bg-primary text-white px-6 py-2.5 rounded-lg text-sm font-medium hover:bg-primary/90 transition-all flex items-center gap-2 disabled:opacity-50"
                        >
                            {saving ? (
